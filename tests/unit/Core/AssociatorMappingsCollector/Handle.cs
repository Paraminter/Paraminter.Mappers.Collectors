...
public sealed class Handle
{
    ...
    public void ValidCommand_TrueReturningModel_AddsMapping()
    {
        ...
        var associator = Mock.Of<ICommandHandler<IAssociateIndividualMappedArgumentCommand<IArgumentData>>>();

        Mock<IWriteOnlyArgumentAssociatorMappings<IParameter, ICommandHandler<IAssociateIndividualMappedArgumentCommand<IArgumentData>>>> mappingsMock = new();
        Mock<IAddMappedArgumentAssociatorCommand<IParameter, ICommandHandler<IAssociateIndividualMappedArgumentCommand<IArgumentData>>>> commandMock = new();

        mappingsMock.Setup(static (mappings) => mappings.TryAddMapping(It.IsAny<IParameter>(), It.IsAny<ICommandHandler<IAssociateIndividualMappedArgumentCommand<IArgumentData>>>())).Returns(true);
        ...
        mappingsMock.Verify(static (mappings) => mappings.TryAddMapping(It.IsAny<IParameter>(), It.IsAny<ICommandHandler<IAssociateIndividualMappedArgumentCommand<IArgumentData>>>()), Times.Once());
    ...
    public void ValidCommand_FalseReturningModel_AddsMappingAndHandlesError()
    {
        ...
        mappingsMock.Setup(static (mappings) => mappings.TryAddMapping(It.IsAny<IParameter>(), It.IsAny<ICommandHandler<IAssociateIndividualMappedArgumentCommand<IArgumentData>>>())).Returns(false);
    ...
    private static Expression<Action<IAssociatorMappingsCollectorErrorHandler<TParameter>>> HandleDuplicateParameterExpression<TParameter>(
